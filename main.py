...
class Config(object):
    ...
    API_ID = int(os.environ.get("API_ID",  "20783440"))
    API_HASH = os.environ.get("API_HASH", "5d95bcc06b5bb466dd8fb0efe2e0ce0a")
    DOWNLOAD_LOCATION = "./DOWNLOADS"
    SESSIONS = "./SESSIONS"

    AUTH_USERS = os.environ.get('AUTH_USERS', '8113432878').split(',')
    for i in range(len(AUTH_USERS)):
        AUTH_USERS[i] = int(AUTH_USERS[i])

    GROUPS = os.environ.get('GROUPS', '-1002621188449').split(',')
    for i in range(len(GROUPS)):
        GROUPS[i] = int(GROUPS[i])

    LOG_CH = os.environ.get("LOG_CH", "-1002621188449")
